...
@interface YlinkInteractiveMapView ()

@property (nonatomic, strong) YlinkSVG* svg;
@property (nonatomic, strong) NSMutableArray* scaledPaths;

@end

@implementation YlinkInteractiveMapView
...
{
    _svg = [YlinkSVG svgWithFile:mapName];
    
    for (YlinkVGPathElement* path in _svg.paths) {
...
- (void)setColors:(NSDictionary*)colorsDict
{
    for(int i=0;i<[_scaledPaths count];i++) {
        YlinkVGPathElement* element = _svg.paths[i];
